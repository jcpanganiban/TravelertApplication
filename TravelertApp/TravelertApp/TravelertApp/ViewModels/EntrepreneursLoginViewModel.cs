...
{
    public class EntrepreneursLoginViewModel : BindableObject
    {
        ...
        public EntrepreneursLoginViewModel(/*INavigation navigation*/)
        {
        ...
        {
            var response = await EntrepreneurApiServices.ServiceClientInstance.LoginUser(Email, Password);

            if (response != null)
            {
                //await App.Current.MainPage.Navigation.PushAsync(new GeneralPublicPage(response.ConsumerUID.ToString()));
                await App.Current.MainPage.Navigation.PushAsync(new EntrepreneurTabbedPage(response.EntrepreneurUID.ToString()));
        ...
        public bool IsEntrepreneurLoggedIn()
        {
